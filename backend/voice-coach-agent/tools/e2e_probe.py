...
PASS = agent joined, spoke audibly, AND delivered observability telemetry.
Exit codes: 0 pass, 1 agent never joined, 2 joined but no audio track,
3 audio track but silent, 4 setup error, 5 telemetry missing.
...
def macos_speech_samples(text: str) -> np.ndarray:
    """Render a deterministic local spoken turn without using another API."""
    with tempfile.TemporaryDirectory(prefix="freewrite-e2e-") as directory:
        aiff = os.path.join(directory, "utterance.aiff")
        wav = os.path.join(directory, "utterance.wav")
        ...
        with wave.open(wav, "rb") as audio:
            return np.frombuffer(audio.readframes(audio.getnframes()), dtype=np.int16).copy()


def mint_token(room_name: str, profile_id: str) -> str:
    metadata = json.dumps({
        "userId": "e2e-probe",
        "context": {
            "entryType": "text",
...
async def main(profile_id: str, user_utterance: str | None) -> int:
    load_dotenv(os.path.join(os.path.dirname(__file__), "..", ".env"))
    url = os.environ["LIVEKIT_URL"]
    room_name = f"freewrite-e2e-probe-{int(time.time())}"
    log(f"room={room_name} url={url} profile={profile_id}")
    ...
    speech = macos_speech_samples(user_utterance) if user_utterance else None
    ...
    def _on_data(packet: rtc.DataPacket) -> None:
        if packet.topic != "freewrite.voice.telemetry":
            return
        try:
            event = json.loads(packet.data)
            telemetry_events.append(event)
            log(f"telemetry: {event.get('eventType')}/{event.get('stage')}")
        except (UnicodeDecodeError, json.JSONDecodeError) as error:
            log(f"telemetry decode failed: {error}")

    def _on_transcript(reader, participant_identity):
        async def read():
            text = await reader.read_all()
            if text.strip():
                transcripts.append((str(participant_identity), text.strip()))
    ...
    log("connecting…")
    await room.connect(url, mint_token(room_name, profile_id))
    ...
    async def feed_silence() -> None:
        ...
        while not stop_feed.is_set():
            if speech is not None and speak_now.is_set() and speech_offset < len(speech):
                count = min(len(data), len(speech) - speech_offset)
                data[:] = 0
                data[:count] = speech[speech_offset:speech_offset + count]
                speech_offset += count
            else:
    ...
    try:
        ...
        if user_utterance:
            log(f"speaking synthetic user turn: {user_utterance}")
            speak_now.set()

        peak_rms = 0.0
        total = 0
        # A native realtime track exists before the user finishes speaking.
        # Keep listening for a full response window after the synthetic turn,
        # rather than counting the user's audio duration against the agent.
        speech_seconds = (len(speech) / SAMPLE_RATE) if speech is not None else 0
        deadline = time.time() + LISTEN_SECONDS + speech_seconds
        ...
        log(f"listened: {total/SAMPLE_RATE:.1f}s of audio, peak RMS={peak_rms:.0f}, "
            f"states={states}")
        agent_transcripts = [text for identity, text in transcripts if identity.startswith("agent")]
        if agent_transcripts:
            log(f"agent said: {' | '.join(t[:160] for t in agent_transcripts[:3])}")

        if peak_rms >= SPEECH_RMS_THRESHOLD:
            if not any(event.get("eventType") == "config" for event in telemetry_events):
                log("FAIL: audible session delivered no config telemetry")
                return 5
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--profile", default="cascade-gemini-3.5-flash",
        help="Canonical voice profile id from coach/config.py",
    )
    parser.add_argument(
        "--user-utterance",
        help="Speak this locally synthesized turn after the agent joins (needed for listen-first profiles).",
    )
    args = parser.parse_args()
    try:
        raise SystemExit(asyncio.run(main(args.profile, args.user_utterance)))
